...
def findPopulatingLevels(_levels, _unique_levels_in, _level_intensities_in, _transitions, _intensities):
    ...
    for counter, level in enumerate(_levels):
        level = round(float(level),0)
        comparison_level = level - round(float(_transitions[counter]),0)
        comparison_level = round(comparison_level,0)
...
def calculateInitialPopulation(_unique_levels_in, _level_intensities_in, _unique_levels_out, _level_intensities_out, _initial_levels, _initial_population):
    ...
    for counter, level in enumerate(_unique_levels_out):
        if level not in _unique_levels_in: #is it a new level?
            total_initial_intensity += _level_intensities_out[counter]# add to the total intensity output
            _initial_levels.append(level)# add to the initial levels
            _initial_population.append(_level_intensities_out[counter])# add to the initial population
        else:
            ...
            #print('{} {} {}'.format(_level_intensities_out[counter] - _level_intensities_in[_unique_levels_in.index(level)], level, _level_intensities_out[counter] - _level_intensities_in[_unique_levels_in.index(level)]))
...
def main():
    ...
    #generateInitialPopulationFile(initial_population, 11, initial_levels, z, a)
