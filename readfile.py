...
class Transition(object):
    def __init__(self, index=0, energy=0 ,Multi="Unknown", ILev=0, FLev=0, intensity=0, ConvCoef=0, BrRatio=0, MixRatio=0, EILev=0, EFLev=0):
        self.index = index
        self.energy = energy
        self.Multi = Multi
        self.ILev = ILev
...
def getLevelEnergy(levelgammaentry):
    return levelgammaentry.initialEnergy

def getLevelIndex(level):
    return level.index

def getTransitionILevel(transition):
    return transition.ILev

def getTransitionFLevel(transition):
    return transition.FLev
...
###############################################################################
# loop over the level list and extract useful information
...
###############################################################################
# loop over the gamma list and extract useful information
for i in range(3,len(gamma_lines),3):
    ...
    gammas.append(Transition(index,energy,Multi,ILev,FLev,intensity,ConvCoef,BrRatio,MixRatio,EILev,EFLev))
...
###############################################################################
# start generating the G4data files here
